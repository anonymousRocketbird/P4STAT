...
switch_add_ipv4_entry(const five_tuple_t *five_tuple, const pipe_sess_hdl_t sess_hdl,
	...
	p4_pd_qoe_switch_classification_ipv4_match_spec_t match_spec = {
	...
	p4_pd_qoe_switch_classification_hit_action_action_spec_t action_spec = {
			register_index,
	};
	pipe_status_t status = p4_pd_qoe_switch_classification_ipv4_table_add_with_classification_hit_action(
			sess_hdl,
			devTarget,
			&match_spec,
			&action_spec,
...
switch_add_ipv6_entry(const five_tuple_t *five_tuple, const pipe_sess_hdl_t sess_hdl,
	...
	p4_pd_qoe_switch_classification_ipv6_match_spec_t match_spec = {
			.ipv6_srcAddr = {
	...
	pipe_status_t status = p4_pd_qoe_switch_classification_ipv6_table_add_with_classification_hit_action(
			sess_hdl,
			devTarget,
			&match_spec,
			&action_spec,
...
switch_remove_ipv4_entry(const pipe_sess_hdl_t sess_hdl, const p4_pd_entry_hdl_t entryHdl) {
	...
	pipe_status_t status = p4_pd_qoe_switch_classification_ipv4_table_delete(sess_hdl,
																			 ALL_PIPES.device_id,
																			 entryHdl);
	if (status) {
...
static int switch_remove_ipv6_entry(const pipe_sess_hdl_t sess_hdl, const p4_pd_entry_hdl_t entryHdl) {
	pipe_status_t status = p4_pd_qoe_switch_classification_ipv6_table_delete(sess_hdl, ALL_PIPES.device_id,
																			 entryHdl);
...
int switch_write_bloom_filter(const pipe_sess_hdl_t sess_hdl, const int pipe_id,
	...
	static bloom_filter_write_fn_t *write_functions[] = {
			...
			p4_pd_qoe_switch_register_write_bloom_filter_2,
			p4_pd_qoe_switch_register_write_bloom_filter_3,
			p4_pd_qoe_switch_register_write_bloom_filter_4
	};

...
#define CLEAR_TABLE(TABLE) { \
    DP_EXIT_ON_ERROR(p4_pd_qoe_switch_##TABLE##_get_entry_count(sess_hdl, ALL_PIPES, &count), \
    	"Error when getting " #TABLE " entry count")	                         \
    int forward_entry_hdls[MAX_FLOWS];\
    p4_pd_qoe_switch_##TABLE##_get_first_entry_handle(sess_hdl, ALL_PIPES, &forward_entry_hdls[0]);\
    if (count > 1) {\
        p4_pd_qoe_switch_##TABLE##_get_next_entry_handles(sess_hdl, ALL_PIPES, forward_entry_hdls[0],\
            (int) count - 1, forward_entry_hdls + 1);\
    }\
    for (int i = 0; i < count; i++) {\
        p4_pd_qoe_switch_##TABLE##_table_delete(sess_hdl, 0, forward_entry_hdls[i]);\
    }\
    p4_pd_qoe_switch_##TABLE##_table_reset_default_entry(sess_hdl, ALL_PIPES);       \
}
...
#define SET_ASSYM(TABLE) { \
    ...
    p4_pd_qoe_switch_##TABLE##_set_property(sess_hdl, 0, PD_TABLE_ENTRY_SCOPE, value, args); \
}
...
//	SET_ASSYM(iat_log_max_table)
	...
	for (int i = 0; i < (sizeof(iat_log_square) / sizeof(log_square_entry_t)); i++) {
		...
		DP_EXIT_ON_ERROR(p4_pd_qoe_switch_iat_compute_log_table_table_add_with_iat_compute_log_action(
	...
	for (int i = 0; i < (sizeof(size_log_square) / sizeof(log_square_entry_t)); i++) {
		...
		DP_EXIT_ON_ERROR(p4_pd_qoe_switch_compute_size_log_table_table_add_with_compute_size_log_action(
				sess_hdl, ALL_PIPES, &match_spec, &action_spec, &entry_hdl
		), "when inserting size_log entry");
	}

	for (int i = 0; i < sizeof(header_length_match) / sizeof(p4_pd_qoe_switch_header_length_table_match_spec_t); i++) {
		DP_EXIT_ON_ERROR(p4_pd_qoe_switch_header_length_table_table_add_with_header_length_hit(
	...
	for (int i = 0; i < sizeof(forward_entries) / sizeof(forward_entry_t); i++) {
		...
		DP_EXIT_ON_ERROR(
				p4_pd_qoe_switch_forward_table_table_add_with_forward_action(sess_hdl, ALL_PIPES, &match_spec,

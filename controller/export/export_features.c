 ...
 */
static inline void print_session_five_tuple(five_tuple_t *five_tuple, FILE *file, int32_t session_id, char *uuid, int32_t flow_count) {
	...
	fprintf(file, "%s;%s;%d;%lu;%d;",uuid, client, five_tuple->sh.to_server, five_tuple->ingress_ts, flow_count);
#ifdef DEBUG
...
//		DEBUG_LOG("Waiting for new_feature_condition")
//		// Wait for condition dataplane_new_features_cond
//		pthread_mutex_lock(&dataplane_new_features_lock);
//		while (!dataplane_new_features) {
//			pthread_cond_wait(&dataplane_new_features_cond, &dataplane_new_features_lock);
//		}
//		dataplane_new_features = false;
//		pthread_mutex_unlock(&dataplane_new_features_lock);
//
//		INFO_LOG("Exporting features")
...
//		pthread_spin_lock(&dataplane_feature_lock);
		...
		for (int i = MIN_SESSION_ID; i < FEATURE_SIZE; i++) {
			int entry_index;
			if ((entry_index = hashtable_copy.feature_index_to_entry_map[i]) == -1) {
				continue;
			}
			int32_t session_id = hashtable_copy.entries[entry_index].value;
			five_tuple_t *five_tuple = &hashtable_copy.entries[entry_index].key;
			int32_t flow_count = hashtable_copy.flow_count[session_id];
